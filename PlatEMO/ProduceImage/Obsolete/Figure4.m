...
%% Choose ph = [@DTLZ1, @IDTLZ1, @DTLZ2, @IDTLZ2]
% mode = 'reg';
mode = 'alt';
% ph = @DTLZ1; 
% ph = @IDTLZ1; 
% ph = @DTLZ2; 
ph = @IDTLZ2; 
Problem = ph(); M = Problem.M; D = Problem.D;
N = 120; [Z, ~] = UniformPoint(N, M);
[PF, ~] = GetPFnRef(Problem, 120);


PreprocessProductionImage(1/2, 0.8, 8.8);
...
if strcmp(mode, 'reg')
    if strcmp(func2str(ph), 'DTLZ1')
        first = [0.5, 0, 0]; second = [0, 0.5, 0]; third = [0, 0, 0.5];
    elseif strcmp(func2str(ph), 'IDTLZ1')
        first = [0.5, 0, 0]; second = [0, 0.5, 0]; third = [0, 0, 0.5];
    elseif strcmp(func2str(ph), 'DTLZ2')
        first = [0.5, 0, 0]; second = [0, 0.5, 0]; third = [0, 0, 0.5];
    elseif strcmp(func2str(ph), 'IDTLZ2')
        first = [0.2, 0.4, 0.99]; second = [0.99, 0.2, 0.4]; third = [0.2, 0.99, 0.4];
    end
elseif strcmp(mode, 'alt')
    if strcmp(func2str(ph), 'DTLZ1')
...
multiplier = 2;
scatter3(ax, PF(:,1), PF(:,2), PF(:, 3), 80*multiplier, ...
                'r', 'filled', 'MarkerEdgeColor','k', ...
                'MarkerFaceAlpha',1, 'MarkerEdgeAlpha',1);

scatter3(ax, EP(:,1), EP(:,2), EP(:, 3), 444*multiplier, ...
                'yellow', 'filled', 'pentagram', 'MarkerEdgeColor','k', ...
                'MarkerFaceAlpha',1, 'MarkerEdgeAlpha',1);

scatter3(ax, c(:,1), c(:,2), c(:, 3), 200*multiplier, ...
...
h_norm = plot3(ax, NaN, NaN, NaN, 'o', ...
                'MarkerSize', legend_marker_size-10, ...
...
set(ax, 'Position', [0.12 0.12 0.76 0.86])
...
if strcmp(func2str(ph), 'DTLZ1')
    ...
    set(ax.XLabel, 'Rotation', 0, 'Position', [0.25, 0.0, 0.0])
    set(ax.YLabel, 'Rotation', 0, 'Position', [0.5, 0.25, 0.0])
    set(ax.ZLabel, 'Rotation', 0, 'Position', [0, -0.05, 0.25])
elseif strcmp(func2str(ph), 'IDTLZ1')
    lims = [0 1];
    set(ax, 'XLim', lims);set(ax, 'YLim', lims);set(ax, 'ZLim', lims);
    ticks = [0 0.5 1];
    ...
    set(ax, 'YTickLabelRotation', 0);
    set(ax, 'ZTickLabelRotation', 0);
    set(ax.XLabel, 'Rotation', 0, 'Position', [0.6, -0.3, 0.0])
    set(ax.YLabel, 'Rotation', 0, 'Position', [1.3, 0.7, 0.0])
    set(ax.ZLabel, 'Rotation', 0, 'Position', [0, 0.05, 1.00])
elseif strcmp(func2str(ph), 'DTLZ2')
    if strcmp(mode, 'reg')
        lims = [0 2];
        set(ax, 'XLim', lims);set(ax, 'YLim', lims);set(ax, 'ZLim', lims);
        ...
        set(ax, 'YTickLabelRotation', 0);
        set(ax, 'ZTickLabelRotation', 0);
        set(ax.XLabel, 'Rotation', 0, 'Position', [0.6, -0.3, 0.0]*2)
        set(ax.YLabel, 'Rotation', 0, 'Position', [1.3, 0.7, 0.0]*2)
        set(ax.ZLabel, 'Rotation', 0, 'Position', [0, 0.05, 1.02]*2)
    else
        lims = [0 2];
        set(ax, 'XLim', lims);set(ax, 'YLim', lims);set(ax, 'ZLim', lims);
        ticks = [0 1 2];
...
filename = sprintf('./%s_EP%d.png', func2str(ph), flag);
