function Offspring = OperatorGAdiff(Problem,Parent,Parameter)
% OperatorGAdiff - Crossover and mutation operators of genetic algorithm.
% OFFSPRINGS WILL HAVE STRICTLY DIFFERENT DECISION VARIABLES FROM THE
% PARENTS
%
...
% Genetic operators for real and integer variables
    ...
    % fprintf("Parent1&2:\n")
    % FormatMatrix("% .10f", Parent1);
    % FormatMatrix("% .10f", Parent2);
    ...
    %% Add random perturbation to ALL offspring
    % Generate random perturbation in range [-1, 1] * scale
    scale = 1e-4;  % Base scale for perturbation
    perturbation = scale * (2*rand(size(Offspring)) - 1);
    
    % Apply perturbation while respecting bounds
    Offspring = Offspring + perturbation;
    Offspring = min(max(Offspring, Lower), Upper);

    % disp("Offspring:");
    % FormatMatrix("% .10f", Offspring);
