...
%--------------------------------------------------------------------------
    ...
    properties(SetAccess = protected)
        ...
        saveName  = '';                 % Custom name for saving (overrides class name)
    end
    methods(Access = protected)
        ...
        %        Algorithm = MOEAD('parameter',4,'save',1)
            ...
            for i = isStr(ismember(varargin(isStr),{'parameter','save','run','metName','outputFcn','saveName'}))
    ...
    methods
        ...
        %GetSaveName - Get the name used for saving results.
        %
        %   name = obj.GetSaveName() returns obj.saveName if it has been
        %   set, otherwise returns class(obj). This allows configurable
        %   algorithms to save with meaningful names.
        ...
        function SetSaveName(obj, name)
        %SetSaveName - Set the name used for saving results.
        %
        %   obj.SetSaveName(name) sets the saveName property. This should
        %   be called by configurable algorithms in their main function.
            obj.saveName = name;
        end
    end
    methods(Access = protected, Sealed)
        ...
        function nofinish = NotTerminatedMod(obj,Population,Offspring)
        %NotTerminated - The function called after each generation of the
        %execution.
        %
        %   obj.NotTerminated(P) stores the population P as the result of
        ...
        %       end
            ...
            num   = max(1,abs(obj.save));
            index = max(1,min(min(num,size(obj.result,1)+1),ceil(num*obj.pro.FE/obj.pro.maxFE)));
            obj.result(index,:) = {obj.pro.FE,Population,Offspring};
            drawnow('limitrate');
            obj.outputFcn(obj,obj.pro);
            nofinish = obj.pro.FE < obj.pro.maxFE;
            assert(nofinish,'PlatEMO:Termination','');
...
% The default output function of ALGORITHM
    ...
    if Problem.FE >= Problem.maxFE
        ...
        elseif Algorithm.save > 0
            ...
            algName = Algorithm.GetSaveName();
            folder = fullfile('Data',algName);
            [~,~]  = mkdir(folder);
            if isprop(Problem, 'ID') && isnumeric(Problem.ID) && ~isnan(Problem.ID)
                file = fullfile(folder,sprintf('%s_%s_M%d_D%d_ID%d_',algName,class(Problem),Problem.M,Problem.D,Problem.ID));
            else
                file = fullfile(folder,sprintf('%s_%s_M%d_D%d_',algName,class(Problem),Problem.M,Problem.D));
            end
